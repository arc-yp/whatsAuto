...
<div class="text-center mt-5 card py-5">
	...
	<strong><?= __tr('Last verified on') ?></strong> <br> <?= formatDate(getAppSettings('product_registration', 'registered_at')) ?> (<?= formatDiffForHumans(getAppSettings('product_registration', 'registered_at')) ?>)
		...
		<div class="mt-5">
		<a >Extended Licnese by Click2api</a>
        ...
        <div class="mt-3">
            ...
            @if ($datetime and $dateInAppTimezone)
                @if ($dateInAppTimezone->lte(now()))
                    ...
                    <h2>{{ $formattedDate }}</h2>
                     <a target="_blank" href="#">{{  __tr('Click here to renew your support') }}</a>
                @endif
            @endif
		</div>
	</div>
...
<?= __tr('Thank you for purchase of our product. Please activate it using Envato purchase code.') ?> <br><small><a target="_blank" href="#"><?= __tr('Where Is My Purchase Code?') ?></a></small>
</div>
